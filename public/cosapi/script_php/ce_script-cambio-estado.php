...
function Conectarse()
{
	if (!($con=mysql_connect("balanceo_basedatos_empresas","script_empresas","script_empresas")))
	{
		echo "Error conectando a la base de datos.";
		exit();
	}
	if (!mysql_select_db("empresas_laravel",$con))
	{
 ...
 while ($row = mysql_fetch_array ($result)){
	...
	$resta 				= 	$hora_actual-$hora_bd;
 	...
 	if ($resta>=3600) {
 		...
 		//echo "</br>";
 		//$query_logout  ="INSERT INTO `empresas_laravel`.detalle_eventos (evento_id, user_id, fecha_evento, ip_cliente, observaciones) ";
	    ...
	    $query_logout  ="INSERT INTO `empresas_laravel`.detalle_eventos (evento_id, user_id, fecha_evento, ip_cliente, observaciones) ";
		$query_logout .="VALUES ('15', '".$user_id."', '".$fecha_actual."', '10.200.74.237','Evento WebServer Logout') ";
		mysql_query($query_logout,$con);

		// insertar login a la media noche por cambios en el panel del agente para agente de madrugada -- 14/10/2016 -- Jasvir Vela
		$query_login  ="INSERT INTO `empresas_laravel`.detalle_eventos (evento_id, user_id, fecha_evento, ip_cliente, observaciones) ";
		$query_login .="VALUES ('11', '".$user_id."', '".$fecha_siguiente."', '10.200.74.237','Evento WebServer Login') ";
		mysql_query($query_login,$con);


		// regresar el agente conectado a su estado orginal
		$query_estado_actual  ="INSERT INTO `empresas_laravel`.detalle_eventos (evento_id, user_id, fecha_evento, ip_cliente, observaciones) ";
		$query_estado_actual .="VALUES ('".$evento_id."', '".$user_id."', '".$fecha_siguiente2."', '10.200.74.237','Evento WebServer Restaurar Estado Inicial') ";
