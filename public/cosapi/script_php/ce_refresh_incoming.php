...
$baseccskills="empresas_reportes";
$bdQstats="empresas_qstats";
...
$skills = "SELECT * FROM empresas_reportes.CC_COLAS ";
$result_skills = mysql_query($skills,$conn);
while($list_skills=mysql_fetch_array($result_skills)){
	$query_incoming 	= "SELECT * FROM empresas_qstats.queue_stats_mv WHERE DATE(DATETIME)=DATE(NOW()) AND queue='".$list_skills['CC_NOMBRE']."'";
	...
	$query_refresh = "UPDATE empresas_reportes.CC_CONSOLIDADO SET CC_ATENDIDAS=".$telefonia_atendida." ,CC_ABANDONADAS_MEN_LIM=".$telefonia_abandon." ,CC_ABANDONADAS=".$info2_abandon." ,CC_ATENDIDAS_MAY_LIM=".$info2_atendida." WHERE CC_NOMBRE='".$list_skills['CC_NOMBRE']."'";
