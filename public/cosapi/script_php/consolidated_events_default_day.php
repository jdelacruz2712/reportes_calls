...
$user='script_empresas';
$pass='script_empresas';
$conn=mysql_connect($host,$user,$pass);

//$proyectos = array ('EMPRESAS', 'CORPORAIVO', 'CALIDAD','BACKOFFICE');
$array_proyectos = array ('EMPRESAS');

$fecha_inicial = date('Y-m-d', strtotime('-1 day')) ;
$fecha_final   = date('Y-m-d', strtotime('-1 day')) ;


foreach ($array_proyectos as  $claro) {
    $AsteriskCDR        = [];
    $Usuarios           = [];
    $Eventos            = [];
    $Eventos_Auxiliares = [];
    ...
    switch($claro){
        case 'EMPRESAS' :
            ...
            $tbl_history_event = 'detalle_eventos_history';
            break;
        /*case 'CORPORAIVO' :
    ...
    $sql_insert_history = 'insert into empresas_laravel.'.$tbl_history_event.' (evento_id,user_id,fecha_evento,ip_cliente,observaciones,date_really,anexo)
    select evento_id,user_id,fecha_evento,ip_cliente,observaciones,date_really,anexo FROM '.$bd_laravel.'.detalle_eventos
